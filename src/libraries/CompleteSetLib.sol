...
library CompleteSetLib {
    ...
    /// @dev Wires up a market's fields against `conditionId`, verifies `key`'s currencies are exactly
    /// the wrapped YES/NO ERC-20s that `conditionId`/`collateralToken` deterministically produce, and
    /// approves the CTF to pull collateral from this contract. See
    /// {ICompleteSetInternalizationHook-registerMarket} for the full external contract; moved here
    /// (alongside the rest of this section) purely to stay under the EIP-170 24KB deployed-code limit.
    function registerMarket(
        ...
        if (market.registered) revert ICompleteSetInternalizationHook.MarketAlreadyRegistered(key.toId());
        _requirePreparedUnresolvedBinaryCondition(conditionalTokens, conditionId);

        _wireMarketFields(
            wrapped1155Factory, conditionalTokens, market, collateralToken, conditionId, yesName, yesSymbol, noName, noSymbol
        );
        ...
        emit ICompleteSetInternalizationHook.MarketRegistered(
            key.toId(), conditionId, address(collateralToken), Currency.unwrap(market.yesCurrency), Currency.unwrap(market.noCurrency)
        );
    }

    function _requirePreparedUnresolvedBinaryCondition(IConditionalTokens conditionalTokens, bytes32 conditionId)
        private
        view
    {
        uint256 outcomeSlotCount = conditionalTokens.getOutcomeSlotCount(conditionId);
        if (outcomeSlotCount != BINARY_OUTCOME_SLOT_COUNT) {
            revert ICompleteSetInternalizationHook.ConditionNotBinary(conditionId, outcomeSlotCount);
        }
        if (conditionalTokens.payoutDenominator(conditionId) != 0) {
            revert ICompleteSetInternalizationHook.ConditionAlreadyResolved(conditionId);
        }
    }

    function _wireMarketFields(
        IWrapped1155Factory wrapped1155Factory,
        IConditionalTokens conditionalTokens,
        ICompleteSetInternalizationHook.Market storage market,
        IERC20 collateralToken,
        ...
        pure
    {
        ...
        if (!matchesForward && !matchesReverse) {
            revert ICompleteSetInternalizationHook.CurrencyMismatch(yesCurrency, key.currency0);
        }
    }

    /// @dev Backstop heuristic: true once either (a) the pool's *current* spot price is already at or
    /// past 1:1 parity in the trade's direction, or (b) this trade's own size, projected against the
    /// pool's currently active tick liquidity, would walk the price at or past parity. (b) is a
    /// same-active-tick approximation (it does not simulate crossing into the next initialized tick) —
    /// deliberately simple, and strictly more conservative than (a) alone, since it only ever routes
    /// *more* trades to the safe CTF backstop, never fewer. `public` (not `internal`) purely so
    /// `SqrtPriceMath`'s routines are deployed once in this library instead of inlined into the hook —
    /// see this library's top-level NatSpec.
    function priceIsAtOrPastParity(
        IPoolManager poolManager,
        PoolId poolId,
        bool zeroForOne,
        uint256 amount,
    ...
    /// @dev Merges as much of a market's YES/NO inventory as currently overlaps, crediting any proceeds
    /// beyond the recovered split cost as surplus. Moved out of the hook contract (alongside the other
    /// functions in this section) purely to stay under the EIP-170 24KB deployed-code limit — see this
    /// library's top-level NatSpec on why `public` library functions, not `internal`, achieve that.
    function mergeIfPossible(
        IConditionalTokens conditionalTokens,
        PoolId poolId,
        ICompleteSetInternalizationHook.Market storage market
    ) public returns (uint256 amountMerged, uint256 surplus) {
        ...
        conditionalTokens.mergePositions(market.collateralToken, bytes32(0), market.conditionId, binaryPartition(), amountMerged);
        ...
        emit ICompleteSetInternalizationHook.InventoryMerged(poolId, amountMerged, surplus);
    }
